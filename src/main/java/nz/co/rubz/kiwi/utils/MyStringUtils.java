...
public class MyStringUtils {
	 ...
	 */
	public static String splitChinese(final int chinese_char_len, final String str, final int sum) throws Exception
	{
	...
	/**
	 * 
	 * @param fromTo  mime参数获取from的值作为参数
	 * @return  mime格式获取邮件发送人的别名，没有别名的则取邮件地址
	 */
	public static String getDisplayOrFrom(String fromTo){
		String title="";
		//过滤前后的[和]
		if(fromTo.startsWith("[")){
			fromTo = fromTo.substring(1);
		}
		if(fromTo.endsWith("]")){
			fromTo = fromTo.substring(0,fromTo.length()-1);
		}
		if(!StringUtils.isEmpty(fromTo)){
			if(fromTo.indexOf(" <")>-1){
				title = fromTo.split(" <")[0];
			}else if(fromTo.indexOf("\"<")>-1){
				title = fromTo.split("\"<")[0];
			}else{
				//过滤前后的<和>
				if(fromTo.startsWith("<")){
					fromTo = fromTo.substring(1,fromTo.length());
				}
				if(fromTo.endsWith(">")){
					fromTo = fromTo.substring(0,fromTo.length()-1);
				}
	...
	public static String convertList2Str(List<String> ids ,String separator){
		String t = "";
		for (String id : ids){
			t = t + id + separator;
		}
		return t.substring(0, t.length()-separator.length());
	}
}
