...
//				c.sendMsg(TestMessageGenerator.genGetUndelieverMessage(mobile,"")); // 自动获取离线消息
	...
	@Test
	public void testClientReport() {
		String mobile0 = mobiles[0];
		ClassuClient c = getLoginedClient(mobile0);
		c.sendMsg(TestMessageGenerator.genReportClientInfoMessage(mobile0 ));
	}

	@Test
	public void testClientSuggest() {
		String mobile0 = mobiles[0];
		ClassuClient c = getLoginedClient(mobile0);
		c.sendMsg(TestMessageGenerator.genSuggestMessage(mobile0 ));
	}
